...
public class GeminiWrapper {
    ...
    public static JSONObject getStream(String url, String headers, String body) {
        try {
            ...
            } catch (JSONException e) { e.printStackTrace(); return new JSONObject(); }

            connection.setDoOutput(true);
            try (OutputStream os = connection.getOutputStream()) { os.write(body.getBytes()); }
            if (connection.getResponseCode() != HttpURLConnection.HTTP_OK) return new JSONObject();
    ...
    public static String getSNlM0e(String cookies) {
        try {
            ...
            if (connection.getResponseCode() != HttpURLConnection.HTTP_OK) return "error";
            try (BufferedReader reader = new BufferedReader(new InputStreamReader(connection.getInputStream()))) {
                StringBuilder response = new StringBuilder();
                String line;
